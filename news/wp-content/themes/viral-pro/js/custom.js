...
$(".astm-search-menu a").append("<p class= 'search-text' style='display: inline;font-size:18px'>Search</p>");
	
$(document).keypress(function(event){
  var keycode = (event.keyCode ? event.keyCode : event.which);
  if(keycode == '13'){
    $(".astm-search-menu a").css("visibility", "hidden");
  }
});
	
	
	$(".is-search-form").css("top", "15px");
	$(".search-icon").css("top", "15px");
	
    $(".search-icon").on("click", function() {
		
		if($(window).width() < 600){
			$(".custom-logo").css("visibility", "hidden");
			$(".is-search-form").css("margin-top", "-26px");
			$(".is-search-form").css("margin-right", "-10px");
			$(".is-menu-wrapper .is-menu").css("margin-top", "0px");
		}
	
		$(".search-icon").css("visibility", "hidden");
		$(".search-text").css("visibility", "hidden");
	...
	$(document).click(function () {
		...
		if ($(".search-icon").css("visibility") == "hidden") {

			if($(window).width() < 600){
				$(".custom-logo").css("visibility", "visible");
				$(".is-search-form").css("margin-top", "");
				$(".is-search-form").css("margin-right", "");
				$(".is-menu-wrapper .is-menu").css("margin-top", "-5px");
			}
    ...
    $('.ht-search-button a').click(function () {
        ...
        setTimeout(function () {
            $('.ht-search-wrapper .search-field');
        }, 1000);
    });
    ...
    if (viral_pro_options.customizer_gdpr_settings && typeof (Cookies) !== 'undefined') {
        ...
        if (typeof (issetPrivacypolicy) == 'undefined' && !JSON.parse(viral_pro_options.customize_preview)) {
			...
			$('#home-bottom-banner-wrapper').css("bottom", "0px");
		}
	...
	jQuery("#wpd-field-submit-0_0").css("width", "100%");
   ...
   $(this).css("display", "inline-block");
		...
		$(this).css("margin-left", "8%");
				...
				jQuery.each(jQuery(".wc_email-wrapper"), function () {
    jQuery(this).prependTo(jQuery(this).parents(".wpd_comm_form"));
			$(this).css("display", "inline-block");	
...
$(document).ready(function() {
	...
	//da umesto 0 bude No comment u pocetnoj featuerd image

	if ($('#singleNewsTwo .vl-post-comment-count').length){
		var x=$('#singleNewsTwo .vl-post-comment-count').html();
	
	x = x.slice(x.length - 1)
	
	
	if(x==="0"){
		$('#singleNewsTwo .vl-post-comment-count').html("no");
	}
	}else{
...
//za Headlines stranu, insertovanje banera u archive widget, izmedju postova
if ($("#headlines").length){
if ($(".baner1").length){
	$('.baner1').attr('style',"grid-row: 3;grid-column-start: 1;grid-column-end: span 3;");
	if($("#headlines article:nth-child(6)").length){
		
		$("#headlines article:nth-child(6)").after($(".baner1").get(0).outerHTML); 
	$(".baner1").eq(1).remove();
	}else{
	
		$(".elementor-posts-container").first().after($(".baner1").get(0).outerHTML); 
	$(".baner1").eq(1).remove();
	}
	...
	if ($(".baner2").length){
	$('.baner2').attr('style',"grid-row: 6;grid-column-start: 1;grid-column-end: span 3;");
	if($("#headlines article:nth-child(12)").length){
		//alert("imadevet")
		$("#headlines article:nth-child(12)").after($(".baner2").get(0).outerHTML); 
		$(".baner2").eq(1).remove();	
		
	}else{
		//alert("nema 9")
		
		$(".elementor-posts-container").first().after($(".baner2").get(0).outerHTML); 
		$(".baner2").eq(1).remove();	
	}
	...
	if ($(".baner3").length){
	$('.baner3').attr('style',"grid-row: 9;grid-column-start: 1;grid-column-end: span 3;");
	if($("#headlines article:nth-child(18)").length){
		//alert("imadevet")
		$("#headlines article:nth-child(18)").after($(".baner3").get(0).outerHTML); 
		$(".baner3").eq(1).remove();	
		
	}else{
		//alert("nema 9")
		
		$(".elementor-posts-container").first().after($(".baner3").get(0).outerHTML); 
		$(".baner3").eq(1).remove();	
	}
...
//za Headlines stranu, insertovanje banera u archive widget, izmedju postova mobilna verzija
if ($("#headlines").length){
if ($(".baner4").length){
	$('.baner4').attr('style',"grid-row: 3;grid-column-start: 1;grid-column-end: span 3;");
	if($("#headlines article:nth-child(6)").length){
		
		$("#headlines article:nth-child(6)").after($(".baner4").get(0).outerHTML); 
	$(".baner4").eq(1).remove();
	}else{
	
		$(".elementor-posts-container").first().after($(".baner4").get(0).outerHTML); 
	$(".baner4").eq(1).remove();
	}
	...
	if($("#headlines article:nth-child(12)").length){
		//alert("imadevet")
		$("#headlines article:nth-child(12)").after($(".baner5").get(0).outerHTML); 
		$(".baner5").eq(1).remove();	
		
	}else{
		//alert("nema 9")
		
		$(".elementor-posts-container").first().after($(".baner5").get(0).outerHTML); 
		$(".baner5").eq(1).remove();	
	}
	...
	if ($(".baner6").length){
	$('.baner6').attr('style',"grid-row: 9;grid-column-start: 1;grid-column-end: span 3;");
	if($("#headlines article:nth-child(18)").length){
		//alert("imadevet")
		$("#headlines article:nth-child(18)").after($(".baner6").get(0).outerHTML); 
		$(".baner6").eq(1).remove();	
		
	}else{
		//alert("nema 9")
		
		$(".elementor-posts-container").first().after($(".baner6").get(0).outerHTML); 
		$(".baner6").eq(1).remove();	
	}
...
//za category stranu, insertovanje banera u archive widget, izmedju postova
if ($("#archPost").length){
if ($(".baner1").length){
	$('.baner1').attr('style',"grid-row: 2;grid-column-start: 1;grid-column-end: span 3;");
	if($("#archPost article:nth-child(3)").length){
		
		$("#archPost article:nth-child(3)").after($(".baner1").get(0).outerHTML); 
	$(".baner1").eq(1).remove();
	}else{
	...
	if ($(".baner2").length){
	$('.baner2').attr('style',"grid-row: 5;grid-column-start: 1;grid-column-end: span 3;");
	if($("#archPost article:nth-child(9)").length){
		//alert("imadevet")
		$("#archPost article:nth-child(9)").after($(".baner2").get(0).outerHTML); 
		$(".baner2").eq(1).remove();	
	...
	//brisanje pocetnih banera
}
	
//Ubacivanje manjih banera za mobilnu verziju za category stranu
	
if ($("#archPost").length){
if ($(".baner3").length){
	$('.baner3').attr('style',"grid-row: 2;grid-column-start: 1;grid-column-end: span 3;");
	if($("#archPost article:nth-child(3)").length){
		
		$("#archPost article:nth-child(3)").after($(".baner3").get(0).outerHTML); 
	$(".baner3").eq(1).remove();
	}else{
	...
	if ($(".baner4").length){
	$('.baner4').attr('style',"grid-row: 5;grid-column-start: 1;grid-column-end: span 3;");
	if($("#archPost article:nth-child(9)").length){
		//alert("imadevet")
		$("#archPost article:nth-child(9)").after($(".baner4").get(0).outerHTML); 
		$(".baner4").eq(1).remove();	
...
function alertFunc() {
	...
	if(window.matchMedia("(max-width: 767px)").matches){
		...
		$(".astm-search-menu.menu-item").remove();  
       
		
	
	
		//sklanja se broj komentara sa mobilne verzije(na search stranici i jos neke..)
		$( ".Mread .elementor-post-avatar").each(function(index,element){
		element.remove();		
...
if ($(".baner1").length){
	...
	$('.baner1').attr('style',"");
	if($("#latest article:nth-child(2)").length){
	
		$("#latest article:nth-child(2)").after($(".baner1").get(0).outerHTML); 
	$(".baner1").eq(1).remove();
	}else{
	...
	$(document).ready(function() {
		...
		//iz nekog razloga zbbog kesiranja, nece da se skloni preloader gif sa firefoxa, ovaj kod resava to
		// ispituje da li je pretrazivac firefox i skloni preloader
	//	if(navigator.userAgent.toLowerCase().indexOf("firefox") > -1){
 // console.log("firefox");		
					  setTimeout(function() { 
$('#ht-preloader-wrap').css("display","none");
		$('#srcIcon').css("margin-bottom", "0px!important");					  
...
$('#btnComm').click();
    ...
    $([document.documentElement, document.body]).animate({
        scrollTop: $("#headDetails").offset().top
    }, 3000);
			
});
		
				$("#TopCasinosIcon").click(function() {
    $([document.documentElement, document.body]).animate({
        scrollTop: $("#TopCasinosHead").offset().top
    }, 2000);});
		
					$("#BonusesIcon").click(function() {
    $([document.documentElement, document.body]).animate({
        scrollTop: $("#BonusesHead").offset().top
    }, 2000);});
		
					$("#GamesIcon").click(function() {
    $([document.documentElement, document.body]).animate({
        scrollTop: $("#GamesHead").offset().top
    }, 2000);});
		
		
		//sklanja zareze iz tagova i mob i desk varijanta
...
/*	function checkIfImageExists(url, callback) {
 ...
 $(".thumbChange").find("img").each(function(){
	 var text = $(this).attr('src');
    var textN = text.replace("desk", "thumb");
  checkIfImageExists(textN, (exists) => {
  if (exists) {
	   $(this).attr('src',textN);
	   $(this).attr('srcset',textN);
...
var maxLength = 65;
    $(".show-read-more").find(".elementor-icon-list-text").each(function(){
        ...
        if($.trim(myStr).length > maxLength){
            ...
            $(this).append('<a href="javascript:void(0);"style="color:#0066cc;font-weight:600;font-family:Barlow;font-size:16px"class="read-more"><span style=color:#333333>...</span>&nbsp&nbsp&nbspSee All ></a>');
            $(this).append('<span style="display:none!important;"class="more-text">' + removedStr + '</span>');
        }
    });
...
$(".hiddenNav").css("display","block");
  ...
  function () {
	...
	$(".hiddenNav a").css("padding-left","60px");
}

});

// Delete hover text on cursor for images
