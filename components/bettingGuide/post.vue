<script setup>
const { imgUrl, category, title, published, slug } = defineProps(['imgUrl', 'category', 'title', 'published', 'slug'])
...
<template>
    <div class="col-xl-4 col-md-6" @click="useRouter().push(`/betting-guides/${slug}`)">
        <article>
            ...
            <h2 class="title">
                <NuxtLink :to="`/betting-guides/${slug}`">{{ title }}</NuxtLink>
            </h2>
            ...
            <div class="d-flex align-items-center">
                ...
                <div class="post-meta">
                    <p class="post-date">
                        <time :datetime="published">{{ month }} {{ day }}, {{ year }}</time>
                    </p>
                </div>
            </div>
